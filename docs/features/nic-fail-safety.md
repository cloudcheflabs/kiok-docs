...
`com.cloudcheflabs.kiok.protocol.internal.InternalNioClient`:
...
```text
-Dkiok.protocol.internal.connect.timeout.seconds=10
-Dkiok.protocol.internal.write.timeout.seconds=10
```

## What changed in the server

`com.cloudcheflabs.kiok.protocol.internal.InternalNioServer`:
...
```bash
docker compose -f tests/docker-compose-kiok.yml up -d
docker pause kiok-worker-2          # silent NIC sim
./tests/test-nic-failure-channel-health.sh    # expect PASS
docker unpause kiok-worker-2
```
