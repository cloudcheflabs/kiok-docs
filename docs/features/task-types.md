...
The same resolution pass that swaps `${conn...}` / `${secret...}` also evaluates `#{ ... }` date functions — `nowFormatted`, `nowMinusFormatted`, `nowPlusInMillis`, etc. — so a daily-scheduled YAML query can use yesterday's date without authoring the DAG in Python or Java. See [Date placeholders](date-placeholders.md) for the full function list and worked examples.
