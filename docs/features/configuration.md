...
| `kiok.scheduler.catchup.max.runs` | `24` | Upper bound on how many runs a single catch-up may create for one DAG after an outage. Only applies to DAGs with `catchup: true`. When the gap is larger, the **most recent** intervals are run and the older ones are dropped with a `WARN` naming the range &mdash; releasing an unbounded backlog at once is an outage of its own. See [Scheduler &amp; Triggers](scheduler.md#catchup-recovering-intervals-missed-during-an-outage). |
