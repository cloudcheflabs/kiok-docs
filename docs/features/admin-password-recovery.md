...
kiok ships with a built-in recovery channel that lets an operator reset the
...
|---|---|---|
| **Master-generated** | `kiok-cli.sh iam:reset-password` | Default. Strong random password printed once on stdout. |
| **Explicit** | `kiok-cli.sh iam:reset-password --new-password 'My!Pass'` | Automation that knows the desired value. Beware: argv may show up in `ps`. |
| **Stdin** | `echo 'My!Pass' \| kiok-cli.sh iam:reset-password --new-password -` | Automation that wants to avoid argv exposure. |
| **Interactive** | `kiok-cli.sh iam:reset-password --interactive` | Operator at a TTY. Prompts for password twice with no echo. |

Resetting a different user is also supported:

```bash
bin/kiok-cli.sh iam:reset-password --user some-user --new-password 'NewPass123'
```
